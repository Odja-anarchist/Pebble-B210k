...
static TextLayer *s_activity_paused_layer;
...
static void activity_load_paused_text_layer(Window *window) {
  GRect bounds = get_window_bounds(window);
  bounds.origin.y = bounds.origin.y + (24);
  bounds.size.h = 28;
  s_activity_paused_layer = text_layer_create(bounds);
  text_layer_set_text_alignment(s_activity_paused_layer, GTextAlignmentCenter);
  layer_add_child(window_get_root_layer(window), text_layer_get_layer(s_activity_paused_layer));
  #if defined(PBL_PLATFORM_EMERY)
    text_layer_set_font(s_activity_paused_layer, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD));
  #else
    text_layer_set_font(s_activity_paused_layer, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD));
  #endif
  text_layer_enable_screen_text_flow_and_paging(s_activity_paused_layer, 3);
}
...
static void redraw_text_layers() {
  ...
  if(isPaused) {
    text_layer_set_text(s_activity_paused_layer, "-Paused-");
  } else {
    text_layer_set_text(s_activity_paused_layer, "");
  }
...
static void activity_load_main_layers(Window *window) {
  ...
  activity_load_paused_text_layer(window);
  redraw_text_layers();
}
...
static void activity_window_unload() {
  ...
  text_layer_destroy(s_activity_paused_layer);
